/** The Meandering Numbers solver. */

function meander_constraint(_id, area_size) {
  /**
   * Generate a constraint for the meandering numbers.
   */
  if (area_size === 1) {
    return `:- area(${_id}, R, C), not number(R, C, 1).`;
  }

  const mutual = `area(${_id}, R, C), area(${_id}, R1, C1), number(R1, C1, N1), adj_4(R, C, R1, C1)`;
  let rule = `meander(${_id}, R, C) :- number(R, C, N), #count { R1, C1 : ${mutual}, |N - N1| = 1 } != 2.`;
  rule += `:- area(${_id}, R, C), number(R, C, 1), #count { R1, C1 : ${mutual}, N1 = 2 } != 1.`;
  rule += `:- area(${_id}, R, C), number(R, C, ${area_size}), #count { R1, C1 : ${mutual}, N1 = ${area_size - 1} } != 1.`; // prettier-ignore
  rule += `:- area(${_id}, R, C), meander(${_id}, R, C), not number(R, C, 1), not number(R, C, ${area_size}).`;
  return rule;
}

modules["meander"] = {
  name: "Meandering Numbers",
  category: "num",
  aliases: ["meanderingnumbers"],
  solve: async function solve(puzzle) {
    solver.reset();
    solver.register_puzzle(puzzle);

    fail_false(puzzle.row === puzzle.col, "This puzzle must be square.");
    const n = puzzle.row;
    solver.add_program_line(grid(n, n));
    solver.add_program_line(adjacent(4));
    solver.add_program_line(adjacent(8));
    solver.add_program_line(avoid_num_adjacent(8));
    solver.add_program_line(unique_num("grid", "area"));

    const areas = full_bfs(puzzle.row, puzzle.col, puzzle.edge);
    for (const [i, ar] of Array.from(areas.keys()).entries()) {
      solver.add_program_line(area(i, ar));
      solver.add_program_line(fill_num(Array.from({ length: ar.length }, (_, i) => i + 1), "area", i)); // prettier-ignore
      solver.add_program_line(meander_constraint(i, ar.length));
    }

    Object.entries(areas).forEach(([_, ar], i) => {
      solver.add_program_line(area({ id: i, src_cells: ar }));
      solver.add_program_line(fill_num({ range: [...Array(ar.length).keys()].map((x) => x + 1), type: "area", id: i }));
      solver.add_program_line(meander_constraint(i, ar.length));
    });

    for (const [point, num] of puzzle.text.entries()) {
      const [r, c, d, pos] = extract_point(point);
      validate_direction(r, c, d);
      validate_type(pos, "normal");
      fail_false(Number.isInteger(num), `Clue at (${r}, ${c}) must be an integer.`);
      solver.add_program_line(`number(${r}, ${c}, ${num}).`);
    }

    solver.add_program_line(display("number", 3));
    await solver.solve();

    return solver.solutions;
  },
  examples: [
    {
      data: "m=edit&p=",
    },
    {
      url: "https://puzz.link/p?meander/15/15/4i894gi914i2944i894gi914i2944i894gi914i294000000vvv000000vvv000000vvv000000vvv000000i7j1j2g8j2j3l1j6j9l6j5j9g5j6j8l3j1j4g9j9j4l2j3j4l2j7j9g8j7j3l8j4j9g5j3j6l1j6j7l1j2j4g6j9j9i",
      test: false,
    },
  ],
};
