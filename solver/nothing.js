/** The All or Nothing solver. */

function nothing_avoid_area_adjacent(color = "black", adj_type = 4) {
  /**
   * Generates a constraint to avoid same color cells on the both sides of an area.
   * An adjacent rule and an area fact should be defined first.
   */
  return `:- area(A, R, C), area(A1, R1, C1), adj_${adj_type}(R, C, R1, C1), A < A1, ${color}(R, C), ${color}(R1, C1).`;
}

modules["nothing"] = {
  name: "All or Nothing",
  category: "loop",
  aliases: ["allornothing"],
  solve: async function solve(puzzle) {
    solver.reset();
    solver.register_puzzle(puzzle);
    solver.add_program_line(grid(puzzle.row, puzzle.col));
    solver.add_program_line(direction("lurd"));
    solver.add_program_line(shade_c("anything"));
    solver.add_program_line(fill_path("anything"));
    solver.add_program_line(adjacent(4));
    solver.add_program_line(adjacent("loop"));
    solver.add_program_line(grid_color_connected("anything", "loop"));
    solver.add_program_line(single_loop("anything"));
    solver.add_program_line(area_same_color("anything"));
    solver.add_program_line(nothing_avoid_area_adjacent("not anything"));
    solver.add_program_line("nothing(A) :- area(A, R, C), not anything(R, C).");

    const areas = full_bfs(puzzle.row, puzzle.col, puzzle.edge);
    for (const [i, ar] of Array.from(areas.keys()).entries()) {
      solver.add_program_line(area(i, ar));
      solver.add_program_line(area_border(i, ar, puzzle.edge));
      solver.add_program_line(count_area_pass(1, i).replaceAll(":-", `:- not nothing(${i}),`));
    }

    for (const [point, draw] of puzzle.line.entries()) {
      const [r, c, _, d] = extract_point(point);
      solver.add_program_line(`:-${draw ? " not" : ""} grid_direction(${r}, ${c}, "${d}").`);
    }

    solver.add_program_line(display("grid_direction", 3));
    await solver.solve();

    return solver.solutions;
  },
  examples: [
    {
      data: "m=edit&p=",
    },
  ],
};
