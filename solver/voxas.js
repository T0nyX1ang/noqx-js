/** The Voxas solver. */

function voxas_constraint(r, c, r1, c1, symbol_name) {
  /**
   * Generate a constraint for the voxas puzzle.
   */
  const tag = tag_encode("belong_to_shape", "voxas", "grid");

  let rule = "";
  if (symbol_name === "circle_SS__1") {
    // white circle, shape and orientation are the same
    rule += `:- ${tag}(${r}, ${c}, T, V), ${tag}(${r1}, ${c1}, T1, V1), |T - T1| + |V - V1| != 0.`;
  }

  if (symbol_name === "circle_SS__2") {
    // black circle, shape and orientation are different
    rule += `:- ${tag}(${r}, ${c}, T, V), ${tag}(${r1}, ${c1}, T1, V1), |T - T1| + |V - V1| != 2.`;
  }

  if (symbol_name === "circle_SS__5") {
    // gray circle, either shape are different or orientation are different
    rule += `:- ${tag}(${r}, ${c}, T, V), ${tag}(${r1}, ${c1}, T1, V1), |T - T1| + |V - V1| != 1.`;
  }

  return rule;
}

modules["voxas"] = {
  name: "Voxas",
  category: "region",
  solve: async function solve(puzzle) {
    solver.reset();
    solver.register_puzzle(puzzle);
    solver.add_program_line(grid(puzzle.row, puzzle.col));
    solver.add_program_line(edge(puzzle.row, puzzle.col));
    solver.add_program_line(adjacent("edge"));
    solver.add_program_line(general_shape("voxas", 2, OMINOES[2]["I"], "grid", "grid", "edge"));
    solver.add_program_line(general_shape("voxas", 3, OMINOES[3]["I"], "grid", "grid", "edge"));
    solver.add_program_line(all_shapes("voxas", "grid"));

    for (const [point, symbol_name] of puzzle.symbol.entries()) {
      const [r, c, d, _] = extract_point(point);
      validate_type(symbol_name, ["circle_SS__1", "circle_SS__2", "circle_SS__5"]);
      fail_false(puzzle.edge.has(point) === true, `Circle must be on an edge at (${r}, ${c}).`);

      if (d === BaseDir.TOP.description && r > 0) {
        solver.add_program_line(voxas_constraint(r, c, r - 1, c, symbol_name));
      }

      if (d === BaseDir.LEFT.description && c > 0) {
        solver.add_program_line(voxas_constraint(r, c, r, c - 1, symbol_name));
      }
    }

    for (const [point, draw] of puzzle.edge.entries()) {
      const [r, c, d, _] = extract_point(point);
      solver.add_program_line(`:-${draw ? " not" : ""} edge_${d}(${r}, ${c}).`);
    }

    solver.add_program_line(display("edge_left", 2));
    solver.add_program_line(display("edge_top", 2));
    await solver.solve();

    return solver.solutions;
  },
  examples: [
    {
      data: "m=edit&p=",
    },
  ],
};
