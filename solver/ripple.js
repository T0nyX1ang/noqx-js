...
function ripple_constraint() {
  /**
   * A constraint for the 'ripples'.
   */
  const row = ":- grid(R, C1), grid(R, C2), number(R, C1, N), number(R, C2, N), (C2 - C1) * (C2 - C1 - N - 1) < 0.";
  const col = ":- grid(R1, C), grid(R2, C), number(R1, C, N), number(R2, C, N), (R2 - R1) * (R2 - R1 - N - 1) < 0.";
  return row + "\n" + col;
}
...
modules["ripple"] = {
  ...
  solve: async function solve(puzzle) {
    ...
    for (const [i, ar] of Array.from(areas.keys()).entries()) {
      ...
      if (flag) {
        solver.add_program_line(`{ number(R, C, (1..${ar.length})) } = 1 :- area(${i}, R, C), not black(R, C).`);
      } else {
        solver.add_program_line(fill_num(Array.from({ length: ar.length }, (_, i) => i + 1), "area", i)); // prettier-ignore
      }
    }

    solver.add_program_line(unique_num(flag ? "not black" : "grid", "area"));
