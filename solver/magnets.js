/** The Magnets solver. */

function magnet_constraint() {
  /**
   * Generate the magnet constraint.
   */
  let constraint = ":- math_G__2(R, C), math_G__2(R1, C1), adj_4(R, C, R1, C1).\n";
  constraint += ":- math_G__3(R, C), math_G__3(R1, C1), adj_4(R, C, R1, C1).\n";
  constraint += ":- math_G__2(R, C), area(A, R, C), not math_G__3(R1, C1), area(A, R1, C1), adj_4(R, C, R1, C1).\n";
  constraint += ":- math_G__3(R, C), area(A, R, C), not math_G__2(R1, C1), area(A, R1, C1), adj_4(R, C, R1, C1).\n";
  constraint += ":- gray(R, C), area(A, R, C), not gray(R1, C1), area(A, R1, C1), adj_4(R, C, R1, C1).\n";
  return constraint.trim();
}

modules["magnets"] = {
  name: "Magnets",
  category: "var",
  solve: async function solve(puzzle) {
    solver.reset();
    solver.register_puzzle(puzzle);
    solver.add_program_line(grid(puzzle.row, puzzle.col));
    solver.add_program_line(shade_cc(["math_G__2", "math_G__3", "gray"]));
    solver.add_program_line(adjacent());
    solver.add_program_line(magnet_constraint());

    const areas = full_bfs(puzzle.row, puzzle.col, puzzle.edge);
    for (const [i, ar] of Array.from(areas.keys()).entries()) {
      fail_false(ar.length === 2, "All regions must be of size 2.");
      solver.add_program_line(area(i, ar));
    }

    for (const [point, num] of puzzle.text.entries()) {
      const [r, c, d, pos] = extract_point(point);
      validate_direction(r, c, d);
      validate_type(pos, "normal");
      fail_false(Number.isInteger(num), `Clue at (${r}, ${c}) must be an integer.`);

      if (r === -1 && c >= 0 && c < puzzle.col) {
        solver.add_program_line(count(num, "math_G__2", "col", c));
      }

      if (r === puzzle.row && c >= 0 && c < puzzle.col) {
        solver.add_program_line(count(num, "math_G__3", "col", c));
      }

      if (c === -1 && r >= 0 && r < puzzle.row) {
        solver.add_program_line(count(num, "math_G__2", "row", r));
      }

      if (c === puzzle.col && r >= 0 && r < puzzle.row) {
        solver.add_program_line(count(num, "math_G__3", "row", r));
      }
    }

    for (const [point, color] of puzzle.surface.entries()) {
      const [r, c, _, __] = extract_point(point);
      fail_false(BaseColor.DARK.includes(color), `Invalid color at (${r}, ${c}).`);
      solver.add_program_line(`gray(${r}, ${c}).`);
    }

    solver.add_program_line(display("math_G__2"));
    solver.add_program_line(display("math_G__3"));
    solver.add_program_line(display("gray"));
    await solver.solve();

    return solver.solutions;
  },
  examples: [
    {
      data: "m=edit&p=",
    },
  ],
};
