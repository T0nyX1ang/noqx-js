...
function slant_rule() {
   ...
   */
  let rule = ':- grid(R, C), grid_direction(R, C, "ul"), not grid_direction(R - 1, C - 1, "dr").\n';
...
function no_loop() {
   ...
   */
  let rule = "reachable(R, C) :- grid(R, C), not grid(R - 1, C - 1).\n";
...
modules["gokigen"] = {
  ...
  solve: async function solve(puzzle) {
    ...
    solver.add_program_line(display("edge_diag_down", 2));
    solver.add_program_line(display("edge_diag_up", 2));
    await solver.solve();
